...
void WFCTARec::SetWFCTAEvent(const std::vector<int>& iisipm, const std::vector<double>& isipmpe, const std::vector<double>& isipmt)
{
	...
	{
		//if(!(iisipm.at(ii)>=4*1024&&iisipm.at(ii)<5*1024))continue;
...
void WFCTARec::SetWFCTAMcEvent(const std::vector<int>& iisipm, const std::vector<double>& isipmpe, const std::vector<double>& isipmt)
{
	double intensity=1;
	std::vector<double> sipmpe_new;
	for(int i=0;i<iisipm.size();i++)
	{
		int sipmid = iisipm.at(i);
		CollectIntensity::Instance()->GetIntensity(sipmid,intensity);
		sipmpe_new.push_back(isipmpe.at(i)/intensity);
	}
	iSipm.clear();
	SipmPe.clear();
...
void WFCTARec::IslandClean()
{
	...
	double MaxPe = -1000, SecondPe = -1000, ThirdPe = -1000;
	int MaxPeSipm = -1000, SecondPeSipm = -1000, ThirdPeSipm = -1000;
	for(int ii=0;ii<iSipm.size();ii++) {  //find bigest sipm
	...
	for(int ii=0;ii<iSipm.size();ii++) {  //find second sipm
		if(0==timeclean.at(ii)){continue;}
		if(MaxPe<SipmPe.at(ii) && MaxPeSipm!=ii) {
			SecondPe=SipmPe.at(ii);
			SecondPeSipm=ii;
		}
	}
	for(int ii=0;ii<iSipm.size();ii++) {  //find third sipm
		if(0==timeclean.at(ii)){continue;}
		if(MaxPe<SipmPe.at(ii) && MaxPeSipm!=ii && SecondPeSipm!=ii) {
			ThirdPe=SipmPe.at(ii);
			ThirdPeSipm=ii;
		}
	}
	if(MaxPeSipm/1024 == SecondPeSipm/1024){
		if(MaxPeSipm!=-1000)
			groupclean.at(MaxPeSipm) = 1;
	}
	else{
		if(SecondPeSipm!=-1000)
			groupclean.at(SecondPeSipm) = 1;
	}
...
void WFCTARec::MergeEvent()
{
	...
	for(int ii=0;ii<iSipm.size();ii++)
	{
		...
		WFCTAMap::Instance()->GetwSipmXY(SIPM, sipm_x, sipm_y);
...
*/
	...
	*/
	...
	//GetEventMapOnFocus(MainTel, core_x, core_y, source_azi, source_zen);
...
void WFCTARec::GetEventMapOnFocus(int main_tel, double core_x, double core_y, double source_azi, double source_zen)
{
	//WFCTAMap::Instance()->SetEventOnFocus(main_tel, iSipm);
	WFCTAMap::Instance()->SetEventOnFocus_New(main_tel, iSipm, core_x, core_y, source_azi, source_zen);
	/*
...
int WFCTARec::GetTriggerTel_Num()
{
	int TriggerTel_Num=0;
	int triggertel[6]={0};
	for(int ii=0;ii<iSipm.size();ii++)
	{
		//if(groupclean.at(ii)<group_cut) continue;
		if(0==groupclean.at(ii)) continue;
		int tel_id = iSipm.at(ii)/1024;
		triggertel[tel_id]=1;
	}
	for(int i=0;i<6;i++)
	{
		TriggerTel_Num += triggertel[i];
	}

	return TriggerTel_Num;
}

void WFCTARec::GetCleanImageOnFocus_old(int main_tel, std::vector<int>& clean_sipm, std::vector<double>& clean_pe, std::vector<double>& clean_x, std::vector<double>& clean_y)
{
	int SIPM, jj;
	double w_imagex, w_imagey;
	double tel_azi, tel_zen;
	...
	for(int ii=0;ii<iSipm.size();ii++)
	{
		...
		double azi, zen;
		WFCTAMap::Instance()->GetSipmAZ(SIPM, azi, zen);
		slaDs2tp(azi, 90*TMath::DegToRad()-zen, tel_azi*TMath::DegToRad(), (90-tel_zen)*TMath::DegToRad(), &w_imagex, &w_imagey, &jj);
		clean_sipm.push_back(SIPM);
		clean_pe.push_back(SipmPe.at(ii));
		clean_x.push_back(w_imagex);
		clean_y.push_back(w_imagey);
		//printf("Clean sipm:%d wfcta focous %lf %lf | %lf %lf\n",SIPM, w_imagex,w_imagey,w_imagex*57.3,w_imagey*57.3);
	}
}
void WFCTARec::GetCleanImageOnFocus(int main_tel, std::vector<int>& clean_sipm, std::vector<double>& clean_pe, std::vector<double>& clean_x, std::vector<double>& clean_y)
{
	for(int ii=0;ii<iSipm.size();ii++)
	{
		...
		int SIPM = iSipm.at(ii);
		double w_imagex, w_imagey;
		WFCTAMap::Instance()->GetwSipmXY(SIPM, w_imagex, w_imagey);
		clean_sipm.push_back(SIPM);
		clean_pe.push_back(SipmPe.at(ii));
		clean_x.push_back(w_imagex);
		clean_y.push_back(w_imagey);
...
void WFCTARec::GetRawImage(std::vector<int>& raw_sipm, std::vector<double>& raw_pe, std::vector<double>& raw_t) const
{
	for(int ii=0;ii<iSipm.size();ii++)
	{
		//if(int(iSipm.at(ii)/1024)==MainTel-1)
		{
			raw_sipm.push_back(iSipm.at(ii));
			raw_pe.push_back(SipmPe.at(ii));
			raw_t.push_back(SipmT.at(ii));
		}
	}
}

void WFCTARec::GetRawImageOnFocus(int main_tel, std::vector<int>& raw_sipm, std::vector<double>& raw_pe, std::vector<double>& raw_x, std::vector<double>& raw_y)
{
	int SIPM, jj;
	double w_imagex, w_imagey;
	double tel_azi, tel_zen;
	...
	for(int ii=0;ii<iSipm.size();ii++)
	{
		//if(int(iSipm.at(ii)/1024)==MainTel-1)
		{
			...
			WFCTAMap::Instance()->GetSipmAZ(SIPM, azi, zen);
			slaDs2tp(azi, 90*TMath::DegToRad()-zen, tel_azi*TMath::DegToRad(), (90-tel_zen)*TMath::DegToRad(), &w_imagex, &w_imagey, &jj);
			raw_sipm.push_back(SIPM);
			raw_pe.push_back(SipmPe.at(ii));
			raw_x.push_back(w_imagex);
			raw_y.push_back(w_imagey);
			//printf("Raw   sipm:%d wfcta focous %lf %lf | %lf %lf\n",SIPM, w_imagex,w_imagey,w_imagex*57.3,w_imagey*57.3);
		}
	}
}
