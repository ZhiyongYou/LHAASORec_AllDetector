...
mcjobdir=/lhaasofs/user/youzhiyong/LHAASO/LHAASORec_AllDetector/drawmcjob

outdir=/eos/user/y/youzhiyong/LHAASO/LHAASORec_AllDetector/showmc
...
do
	...
	do
		...
		do
			...
			echo 'mkdir -p $outdir/wfcta_focus' >>$mcjobdir/$Partical/$energy/run${irun}.sh
			echo 'mkdir -p $outdir/km2a' >>$mcjobdir/$Partical/$energy/run${irun}.sh
			echo "" >>$mcjobdir/$Partical/$energy/run${irun}.sh
			...
			echo './eventshowmc $mclistdir/$ifilelist $outdir' >>$mcjobdir/$Partical/$energy/run${irun}.sh
