	...
	while(infilelist.good())
	{
		...
		for(int iEntry=0;iEntry<n_cosmics;iEntry++)
		{
			...
			//wfcta&km2a combine rec
			{
				...
				DistanceToXmax_K = lhaasorec->GetDistanceToXmax(MainTel, MeanZen, sourcezen_km2a, corex_k, corey_k);
				DistSourceToMean_K = lhaasorec->GetDistSourceToMean(MeanZen, MeanAzi, sourcezen_km2a, sourceazi_km2a);
